...
\begin{titlepage}
  ...
  \fbox{%
    \parbox{0.85\linewidth}{%
      ...
      (100\,K para texto e imagen; $\sim$100\\,K para audio).
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
  \item Medir el rendimiento del índice invertido personalizado bajo tres
        escalas de carga (1\,K, 10\,K y 100\,K documentos/archivos).
...
\begin{table}[H]
  ...
  \begin{tabular}{lp{4.5cm}rp{2cm}p{3.5cm}}
    ...
    \midrule
    Audio  & FMA 100K WAV Dataset — local \texttt{datasets/audio/Data/genres\_original/}
           & 1\,000 WAV & 1.3\,GB & 10 géneros musicales (100 por género) \\
    \midrule
    Imagen & Fashion200K — HuggingFace \texttt{Marqo/fashion200k}
...
\paragraph{FMA 100K WAV Dataset (Audio).}
...
sobre cada archivo de audio; cada cancion se resume en un histograma acustico.
\textbf{Muestreo por escala}: primeras $N$ canciones del directorio ordenado:
1\,000, 10\,000 y 100\,000 archivos de audio.
\textbf{Nota}: la escala máxima se define por archivos de audio, no por ventanas internas.
...
\paragraph{Fashion200K (Imagen).}
Subconjunto reducido de ImageNet con imágenes de baja resolución.
Fuente: HuggingFace \texttt{Marqo/fashion200k}
...
% =============================================================================
\section{Resultados: Modalidad Audio (FMA)}
% =============================================================================
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
  \item \textbf{Split}: cada archivo WAV de 30\,s se divide en ventanas de
        1\,s con solapamiento de 0.5\,s para extraer MFCC; cada archivo se resume en un histograma acustico.
...
Las escalas evalúan:
\begin{itemize}
  \item \textbf{1K}: 1\,000 archivos de audio.
  \item \textbf{10K}: 10\,000 archivos de audio.
  \item \textbf{100K}: 100\,000 archivos de audio.
\end{itemize}

\noindent\textbf{Nota sobre el límite de 100K (no 100K):}
FMA contiene exactamente 1\,000 canciones de 30\,s distribuidas en 10 géneros
(100 por género). Con ventanas de 1\,s y solapamiento de 0.5\,s se generan
100K archivos de audio en la escala maxima de FMA.
Esta es una \textbf{limitación del dataset}, no del sistema:\footnote{%
  El mismo \texttt{AudioSearchIndex} indexa 100\,K archivos sin ningún
  cambio si existiesen más canciones disponibles en FMA.}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrrrrrr}
    ...
    100K  & 1\,000 & 559.2 & 1.657 & 1.720 & 604    & 0.432      & 36.9  & 6\,089.5 \\
    \bottomrule
  \end{tabular}
  \label{tab:audio-custom}
\end{table}

\noindent\textbf{Observaciones (1K, 10K y 100K):}
\begin{itemize}
  ...
  \item La \textbf{Precisión@10 permanece alta en 10K} (0.990, solo 1\% pérdida),
        ...
        En 100K desciende a 0.432, reflejo de mayor confusión inter-género
  ...
  \item El throughput a 10K (3\,323 QPS) es suficiente para búsqueda musical
        real. A 100K (604 QPS) sigue siendo viable para sistemas no críticos en latencia.
...
\begin{figure}[h!]
    ...
    \caption{Latencia promedio de consulta — AudioSearchIndex vs. pgvector HNSW (FMA).}
...
\begin{figure}[h!]
    ...
    \caption{Throughput (QPS) del AudioSearchIndex por escala (FMA).}
...
\begin{table}[H]
  ...
  \begin{tabular}{llrrrr}
    ...
    \midrule
    \multirow{2}{*}{100K} & AudioSearchIndex (propio) & 999   & \win{1.710}  & \win{586}     & 0.432 \\
...
propio: P@10=1.000 en 1K y 10K (vs.\ 0.990 del índice propio a 10K), y 0.460 en 100K
...
\begin{figure}[h!]
    ...
    \caption{Precisión@10 del AudioSearchIndex por escala (FMA).}
...
% =============================================================================
\section{Resultados: Modalidad Imagen (Fashion200K)}
% =============================================================================
...
\noindent Dataset: \textbf{Fashion200K} (HuggingFace \texttt{Marqo/fashion200k}),
...
\begin{table}[H]
  \centering
  \caption{Rendimiento del VisualSearchIndex — Fashion200K (50 queries, K=10)}
  \begin{tabular}{lrrrrrr}
    \toprule
...
Fashion200K (una misma categoría de ropa puede contener imágenes visualmente muy distintas).

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{image_latency.png}
    \caption{Latencia promedio — VisualSearchIndex vs. pgvector HNSW (Fashion200K).}
...
\begin{figure}[h!]
    ...
    \caption{Precisión@10 del VisualSearchIndex por escala (Fashion200K).}
...
\begin{itemize}
  ...
  \item \textbf{Audio (100K)}: P@10 = \textbf{0.432} — degradación esperada en corpus masivo.
  ...
  \item \textbf{Imagen (Fashion200K, 1K--100K)}: P@10 se debe regenerar con el dataset actualizado
...
y se debe volver a medir en imagen con Fashion200K para reportar la precisión final.
En audio, la precisión cae de 1.000 (1K) a 0.432 (100K) por confusión inter-género real.
...
\begin{itemize}
  \item \textbf{Texto (AG News)}: 1K/10K/100K documentos — vs.\ GIN.
  \item \textbf{Audio (FMA)}: 1K/10K/100K canciones — vs.\ pgvector HNSW.
  \item \textbf{Imagen (Fashion200K)}: 1K/10K/100K imágenes — vs.\ pgvector HNSW.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabular}{llll}
    \toprule
    \textbf{Métrica} & \textbf{Texto (AG News)} & \textbf{Audio (FMA)} & \textbf{Imagen (TinyIN)} \\
    \midrule
    \textbf{Latencia $\leq$10K}  & \win{Índice (0.83--8.87ms)} & \win{Índice (0.04--0.30ms)} & \win{Índice (1.67--18.5ms)} \\
    \textbf{Latencia 100K/100K}   & PostgreSQL GIN (9.5ms)      & \win{Índice (1.71ms)}       & \win{PostgreSQL HNSW (10.4ms)} \\
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
  \item \textbf{Escala máxima de audio --- justificación explícita}:
        FMA (\emph{Georgia Tech Zambia Audio Noise dataset}) es el estándar
        ...
        100\,000 archivos de audio.
        \textbf{Esta es una limitación del dataset, no del sistema}: el mismo
        \texttt{AudioSearchIndex} indexa 100\,K archivos sin ningún cambio de
  ...
  \item \textbf{Precisión imagen}: Fashion200K debe evaluarse con el nuevo ground truth de categoría
...
\begin{itemize}
  \item Audio: \texttt{datasets/audio/Data/genres\_original/} (1\,000 WAV, FMA)
  \item Texto: \texttt{experiments/data/text\_*.json} (AG News, manifiestos)
  \item Imágenes: \texttt{data/full/fashion200k/} (100K JPGs Fashion200K)
