...
class AudioSearchIndex:
    """In-memory search index for acoustic codeword histograms."""
    ...
    def add_record(self, record: dict[str, Any]) -> None:
        """Add one audio histogram record to the acoustic index."""
        self.add_histogram(validate_histogram_record(record))

    def add_histogram(self, record: HistogramRecord) -> None:
        """Store one validated audio histogram."""
        if record.modality != "audio":
            raise ValueError("audio search only accepts audio histograms")
    ...
    def search(self, query_histogram: np.ndarray, k: int = 10) -> list[SearchResult]:
        """Return audio chunks ranked by ascending L2 distance."""
    ...
    def __len__(self) -> int:
        """Return the number of indexed audio chunks."""
